...
function RiskScatterChart({ allChapters = [] }) {
  ...
  if (!data.length) return <EmptyChart label="No chapter progress data" />;
...
function SeriesStatusDonut({ seriesStatusBreakdown = {} }) {
  const data = useMemo(() => {
    return Object.entries(seriesStatusBreakdown)
  ...
  if (!total)
    return <EmptyChart label="No series assigned" height="h-[160px]" />;
...
function ReviewQueueAgingChart({ chaptersInReview = [] }) {
  ...
  if (!data.length)
    return <EmptyChart label="No chapters in review" height="h-[160px]" />;
